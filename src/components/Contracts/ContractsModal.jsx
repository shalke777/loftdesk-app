...
import { X, Plus, Search, FileCheck, CheckCircle2, Clock, Download, Trash2, Edit2, ChevronDown } from 'lucide-react';

const fmt = (n) => Number(n||0).toLocaleString('pl-PL', { minimumFractionDigits:2 }) + ' zł';

export function ContractsModal({ open, onClose, contracts=[], onRemove, onMarkAsSigned, onGeneratePDF, onCreateNew, onEdit }) {
    ...
    <div style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(15,23,42,0.7)', display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'24px 16px', overflowY:'auto' }}>
      <div style={{ width:'100%', maxWidth:900, background:'white', borderRadius:20, boxShadow:'0 25px 60px rgba(0,0,0,0.3)', overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:1, background:'#f1f5f9' }}>
          {[
            { label:'Wszystkie',    value:String(contracts.length), color:'#1e293b' },
            { label:'Podpisane',    value:String(signedCount),      color:'#16a34a' },
            { label:'Łączna wartość', value:fmt(totalValue),        color:'#d97706' },
          ].map((k,i) => (
        ...
        <div style={{ maxHeight:420, overflowY:'auto' }}>
          {filtered.length === 0 ? (
            ...
            <div key={c.id||c.number} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 20px', borderBottom:'1px solid #f8fafc', background:'white', transition:'background 0.1s' }}
              ...
              <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                ...
                {onEdit && (
                  <button onClick={()=>{ onEdit(c); onClose(); }} title="Edytuj"
        ...
        <div style={{ padding:'14px 20px', borderTop:'1px solid #f1f5f9', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <span style={{ fontSize:13, color:'#64748b' }}>Łącznie: {contracts.length}</span>
