...
import { Plus, Trash2, FileText, DollarSign, Calculator, X } from "lucide-react";
...
export const CostingPanel = ({
  ...
  const rows = useMemo(() => {
    return lines.map((line) => {
      ...
      if (!rate) {
        return {
          ...line,
          name: "BŁĄD: Brak pozycji w cenniku",
          unit: "?",
          unitNet: 0,
          net: 0,
          vat: 0,
  ...
  const availableRates = useMemo(() => {
    return Object.entries(rates)
      .sort((a, b) => {
        const catCompare = a[1].category.localeCompare(b[1].category, "pl");
      ...
      {showCustomForm && (
        <div className="edit-form-card" style={{ borderColor: '#16a34a', marginBottom: '16px' }}>
          <h3 className="edit-form-title" style={{ color: '#16a34a' }}>
            <Plus size={20} />
            Dodaj własną pozycję (bez dodawania do cennika)
          </h3>
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <select
                className="input"
                value={customForm.vat}
                onChange={(e) => setCustomForm({ ...customForm, vat: parseFloat(e.target.value) })}
              >
        ...
        >
          ...
          {availableRates.map(([code, rate]) => (
            <option key={code} value={code}>
              [{rate.category}] {rate.name} - {currency(rate.priceNet)}/{rate.unit}
            </option>
          ))}
        </select>
        <button
      ...
      {lines.length === 0 ? (
        <div className="empty-state" style={{ padding: '40px 20px' }}>
          ...
          <div style={{ fontSize: '14px', color: '#94a3b8', marginTop: '8px' }}>
            Wybierz z cennika lub dodaj własną pozycję
          </div>
        </div>
      ) : (
        ...
        <div style={{ marginTop: '20px' }}>
          <div className="grid-4">
            ...
            <div className="summary-box summary-neutral">
              <div className="summary-label">Materiały (30%)</div>
              <div className="summary-value">{currency(summary.materials)}</div>
            </div>
          </div>
